...
`claude-notify resurrect save` SHALL enumerate all tmux panes (across all sessions) whose `pane_current_command` starts with `claude`. For each such pane it SHALL record: `tmux_session`, `window_name`, `pane_index`, `session_id`, and `project_path`. The result is written atomically to the resurrect sidecar. `window_name` is used instead of `window_index` because window indices shift when windows are added, removed, or reordered.

#### Scenario: Active panes persisted
- **WHEN** one or more panes are running `claude*`
- **THEN** each pane is written as an entry in the sidecar with `window_name`, `pane_index`, `session_id`, and `project_path`
