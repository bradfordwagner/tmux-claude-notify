...
The session index is the JSONL file at `~/.local/share/tmux-claude-notify/sessions.jsonl` that persists per-session metadata across dashboard sessions. It tracks all discovered Claude sessions, their real filesystem paths, live pane state, pin status, and last activity. This capability covers the record schema and the store API (Upsert, ReadAll, SetPinned, Compact, DiscoverAll, RecoverPath).
