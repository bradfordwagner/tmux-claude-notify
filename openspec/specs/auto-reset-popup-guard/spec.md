# Spec: auto-reset-popup-guard

## Purpose

When the auto-reset subprocess wakes after its configured delay, it checks whether the grimoire shpell popup (claude-notify dashboard) is currently open before clearing a notification. If the popup is live, the auto-reset is skipped entirely, leaving the notification for explicit dashboard dismissal. This prevents notifications from vanishing while the user is actively managing them.
