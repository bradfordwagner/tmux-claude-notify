...
- **THEN** the notified pane is focused via `SelectPane` + `SelectWindow`
...
The TPM option `@claude-notify-jump-key` SHALL control the keybinding for `claude-notify jump`, defaulting to `C-M-P`. The binding SHALL always use `run-shell` (not grimoire shpell) since jump is non-interactive.

#### Scenario: Default keybinding
- **WHEN** `@claude-notify-jump-key` is not set in `tmux.conf`
- **THEN** `C-M-P` is bound to `run-shell '<binary> jump'`
