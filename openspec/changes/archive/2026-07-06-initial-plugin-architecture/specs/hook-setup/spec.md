...
- **AND** the dashboard shows `[Stop] hooks configured`
...
The `hookEvents` slice in `setup.go` is the single source of truth — adding a new event name there is sufficient to include it in both the check and configure paths.
